...
def main():
    ...
    if percent_sbd > 10:
        ...
        log.info('New GOLOS amount on full convertation by blockchain price: {:,.0f} GOLOS'.format(converted_supply))

        converted_supply = sbd_supply.amount / median_estimated
        log.info('New GOLOS amount on full convertation by feed price: {:,.0f} GOLOS'.format(converted_supply))
        log.info(
    ...
    golos_reward_fund = (
        total_reward_fund_steem.amount / 2 + total_reward_fund_steem.amount / 2 * (10000 - sbd_print_rate) / 10000
    )
